...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
init([]) ->
  ?MODULE = ets:new(?MODULE, [set, named_table, protected]), % Starting protected ETS table
  {ok, ?MODULE}.
...
handle_call({register, Name, Pid}, _From, Tid) ->
  MatchSpec = ets:fun2ms(fun({N,P,_Ref}) when N==Name; P==Pid -> {N,P} end), % Building match spec via parse transform
  case ets:select(Tid, MatchSpec) of
    [] ->
      % free to insert. We monitor the process we have registered (to unregister it in case of failure)
      % and then add the entry to the table.
...
handle_call({unregister, Name}, _From, Tid) ->
  % find the monitor reference (with a lookup on the name), cancel the monitor, then delete the entry and keep going.
...
%% One of the processes we were monitoring died.
%% When that happens, we delete the entry based on the reference we have in the message, then move on.
