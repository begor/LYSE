...
% Synchronous version for clients
% notice that we do not reply to the client yet.
% This is because we have nothing interesting to say, and we want the client locked and
% waiting for the trade to be accepted before doing anything.
...
idle_wait({ask_negotiate, OtherPid}, S=#state{other = OtherPid}) ->
  gen_fsm:reply(S#state.from, ok), % Only here we reply to client, cause we know that negotiate acccepted
...
% This one should be synchronous because we don't want the user
% to keep modifying his offer by adding items while claiming he's ready
...
ready(ack, S=#state{}) ->
  case priority(self(), S#state.other) of
    true ->
      % Notice two-phase commit protocol
      try
...
% Prioritizes PIDs for we can have one process starting the commit and the other following the orders.
